...
export function buildStreamArgs(playlist:MediaInfo[],concatFile:string|undefined,o:OutputConfig,cfg:AppConfig,encoder:Encoder,opts:StreamBuildOptions={}){
 ...
 if(scene==='video'){
  ...
  a.push('-re','-f','lavfi','-i',`color=c=#07111f:s=${o.width}x${o.height}:r=${o.fps}`);audioIndex=1;a.push('-f','lavfi','-i','anullsrc=channel_layout=stereo:sample_rate=44100');
 }
 if(cfg.overlay.imageEnabled&&cfg.overlay.imagePath&&fs.existsSync(cfg.overlay.imagePath)){overlayIndex=audioIndex+1;a.push('-loop','1','-framerate','1','-i',cfg.overlay.imagePath)}
 const W=o.width,H=o.height,p=pos(cfg.overlay.position,W,H),parts:string[]=[];let base=buildFilter(o);
 if(scene!=='video'){const t=escText(sceneText(cfg,scene));base+=`,drawtext=text='${t}':fontcolor=white:fontsize=${Math.max(34,Math.round(H/16))}:x=(w-text_w)/2:y=(h-text_h)/2:box=1:boxcolor=black@0.35:boxborderw=24`}
