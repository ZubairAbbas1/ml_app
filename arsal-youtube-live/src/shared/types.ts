...
export interface AppConfig{mode:StreamMode;loop:boolean;autoReconnect:boolean;encoder:Encoder;audioEnabled:boolean;audioVolume:number;rememberKeys:boolean;recordLocal:boolean;recordDir:string;horizontal:OutputConfig;vertical:OutputConfig}
export interface StreamStats{state:StreamState;fps:number;bitrateKbps:number;speed:string;frames:number;elapsed:number;reconnects:number;lastError?:string}